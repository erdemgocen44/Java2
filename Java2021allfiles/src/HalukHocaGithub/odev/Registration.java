...
public class Registration {
    List<_12_dateTime.odev.User>users=new ArrayList<>();
    /*
    ...
    public List<_12_dateTime.odev.User> register(){
        Scanner scan =new Scanner(System.in);
        System.out.println("Lütfen adinizi giriniz");
        String isim=scan.nextLine();
        _12_dateTime.odev.User user=new _12_dateTime.odev.User(isim, LocalDateTime.now());
        users.add(user);
        return users;
    }

    public void listele(List<_12_dateTime.odev.User>users) {
        for (_12_dateTime.odev.User each : users) {
            System.out.println("Adi: "+each.name+" kayit zamani "+each.registerDate);
        }
    }
    public void printHappyUsers(List<_12_dateTime.odev.User> users) {
        for (User each : users) {
